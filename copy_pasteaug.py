...
class CopyPasteV2(A.DualTransform):
    ...
    '''
    def __init__(self, data_root="../input/data", json_dir="../input/data/train.json", \
    ...
    def choice_index(self):
        ...
        if w > self.max_resize or h > self.max_resize:
            new_w = math.ceil(w * (random_resize / max(w,h)))
            new_h = math.ceil(h * (random_resize / max(w,h)))
